server <- function(input, output) {
  output$Timelines<-renderPlot({
    ...
    table<-data.frame(year=rep(0:input$year,3),value=c(no_contrib,fixed_contrib,growing_contrib),variable=as.factor(rep(c('no_contrib','fixed_contrib','growing_contrib'),each=input$year+1)),ncol=3)
    if(input$facet=='No'){  
      ggplot(table)+
